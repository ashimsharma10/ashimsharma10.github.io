...
draft: false
summary: 'A conceptual guide to MLOps tooling — MLflow, artifact versioning, reproducible pipelines, framework integrations, alternatives, and deploying and improving models in production.'
---
...
<style>{`
.logo-row{display:flex;flex-wrap:wrap;gap:22px 34px;align-items:flex-end;justify-content:center;margin:26px 0 10px}
.logo-cell{display:flex;flex-direction:column;align-items:center;gap:10px}
.logo-cell img{height:44px;width:auto;display:block}
.logo-cell span{font-size:13px;font-weight:600;opacity:.7}
`}</style>
...
- **Aliases and tags** mark which version is which. An alias is a movable pointer like `champion`, `challenger`, or `eu-prod` attached to a specific version; serving references the *alias*, never a hardcoded path.
- Promotion is a **metadata move** (repoint `champion` from v3 to v4), so deploying is one operation and **rollback is just repointing back**.
- The registry is also the home for governance: approvals, descriptions, and an audit trail of who promoted what, when.

> **Note:** Older MLflow used fixed **stages** (`Staging` / `Production` / `Archived`). Those are now deprecated in favor of aliases and tags, which are far more flexible once you serve several variants — regions, cohorts, champion/challenger — at once. New systems should use aliases.
...
> **Feature stores** (Feast, Tecton, Databricks) exist for the hardest artifact of all: the features themselves. They hold one definition of each feature and serve it *identically* offline for training and online for inference — the cleanest structural fix for the train/serve skew flagged above.
...
> **In practice at scale:** MLflow Projects is lightweight and rarely the orchestrator in a large lab. Pipelines usually run on a dedicated orchestrator — Airflow, Kubeflow Pipelines, Metaflow, Argo, Dagster, or Prefect — with MLflow handling tracking and the registry *inside* each step. The principle is identical; only the runner changes.
...
> **Reality check:** bit-for-bit reproducibility on GPUs is often impractical anyway — nondeterministic kernels, atomic adds, and driver differences make exact reruns rare. The achievable goal is *provenance* (knowing exactly what produced a model) plus *close* reproduction, not identical bits.
...
<div className="logo-row">
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/pytorch.svg" alt="PyTorch logo" /><span>PyTorch</span></div>
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/tensorflow.svg" alt="TensorFlow logo" /><span>TensorFlow / Keras</span></div>
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/huggingface.svg" alt="Hugging Face logo" /><span>HuggingFace</span></div>
</div>
...
**The through-line:** the more a framework owns its training loop, the more can be captured for free. PyTorch trades automation for control; Keras trades control for automation; HuggingFace lands in the middle. One caveat at scale — autologging is greedy, capturing every metric and large artifacts on every run, so on big sweeps teams narrow it down and log deliberately to keep the tracking store fast and cheap.
...
<div className="logo-row">
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/mlflow.svg" alt="MLflow logo" /><span>MLflow</span></div>
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/wandb.svg" alt="Weights & Biases logo" /><span>Weights & Biases</span></div>
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/dvc.svg" alt="DVC logo" /><span>DVC</span></div>
  <div className="logo-cell"><img src="/static/images/mlops-tooling-from-experiment-tracking-to-production/neptune.svg" alt="Neptune logo" /><span>Neptune</span></div>
</div>
...
The crucial property in all three: **the environment travels with the model**, so the serving container is built from the same environment it trained in — closing the train/serve gap.

Not everything is a live endpoint, either. A large share of production inference is **batch** (score a table on a schedule) or **streaming** (score events off a queue); the same bundle drops into all three. Match the serving mode to the latency the product actually needs — always-on GPU endpoints are the most expensive option, and many use cases don't need one. In a mature setup, CI/CD sits between registry and production and rolls out *gradually*:
...
```mermaid
flowchart TD
    PM["Production Model<br/>serving live traffic"] --> PRED["Predictions"]
    PRED -->|"labels arrive days to weeks later"| LAB["Ground-Truth Labels"]
    PRED --> MON{"Monitoring<br/>drift · latency · errors"}
    LAB --> MON
    MON -->|"healthy"| PM
    MON -->|"threshold breached"| RT["Retrain Pipeline"]
    RT --> REG["Model Registry<br/>new version"]
    REG -->|"promote"| PM
    style PM fill:#DBEAFE,stroke:#2563EB,color:#111,font-size:15px
    style PRED fill:#D9F2EC,stroke:#1F7A66,color:#111,font-size:15px
    style LAB fill:#D9F2EC,stroke:#1F7A66,color:#111,font-size:15px
    style MON fill:#FDE8E8,stroke:#C0392B,color:#111,font-size:15px
    style RT fill:#FFF4D6,stroke:#B7791F,color:#111,font-size:15px
    style REG fill:#DBEAFE,stroke:#2563EB,color:#111,font-size:15px
```
