...
draft: false
summary: 'What graph engineering is, how it differs from loops and workflow engines, when to build one, a refund approval worked through end to end, and the failure modes nobody warns you about.'
---
...
Once every box is a full agent, drawing the boxes and arrows stops being plumbing and becomes org design. That is the entire argument behind the new name.
...
**Versus free-form multi-agent chat.** In a chat-style system, agents message each other and the conversation decides what happens. It demos beautifully and operates terribly: nobody can say why a run cost $40, took 11 minutes, and produced the wrong answer. A graph gives up some flexibility to get back the thing you need in production, which is a route you can point at afterwards.
...
- Different stages want genuinely different tools, models, or permissions.
...
The honest failure mode of this whole discipline is **over-graphing**: freezing a workflow into boxes and arrows before you understand it, then spending months maintaining a rigid structure that a plain loop handled better.
...
Notice what these have in common: a step where something is checked, and a defined thing that happens when the check fails.

## One example end to end: a refund that must not go wrong

Take the clearest case of all, where a graph is not just tidier but the only sane option: **a customer asks for a refund, and money is about to move.**
...
That two day gap in the middle is the whole point. Run this as a single agent in a loop and you hit five problems at once:

- **The pause kills it.** A loop waiting two days is a chat session someone has to keep alive. The graph simply stopped between two boxes and resumed with its state intact.
...
The flip side, so this does not read as an advertisement: if every refund under 20 USD were auto-approved with no human, no fraud path, and no compliance requirement, this whole diagram collapses to one agent with two tools, and that is the correct design. The graph earns its complexity because of the pause, the permissions, and the audit, not because it looks impressive.
...
The nodes and arrows are the fun part. The thing that decides whether your graph survives contact with production is what you let travel between the boxes.
...
This is where graphs earn their keep. Because progress lives in named state rather than a chat window, a run can be saved between any two boxes.
...
The problems are as predictable as the patterns.
...
- **State bloat.** Everything gets stuffed into shared state until each hop carries a novel. Pass pointers, not payloads.
...
"What broke after the config change?" walks the cause and time links. "What do we know about service X?" walks the meaning and entity links. Same stored memories, different paths through them, better answers than one flat pile of embeddings.
