...
**[affaan-m/everything-claude-code](https://github.com/affaan-m/ECC)** (ECC) — the largest community configuration framework, and one of the most-starred repos in the entire space. It bundles everything this guide covers into one installable kit: 67 agents, 278 skills, hook events, security scanning, and memory persistence, refined over 10+ months of daily production use. Its notable twist is that it's harness-agnostic — the same skills and agents run not only in Claude Code but in Cursor, Codex, OpenCode, Gemini CLI, and others — making it a good reference for how portable the skill format has become.
